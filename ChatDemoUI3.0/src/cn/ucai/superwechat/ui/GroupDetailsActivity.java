...
import cn.hyphenate.easeui.widget.EaseExpandGridView;
...
public class GroupDetailsActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		RelativeLayout clearAllHistory = (RelativeLayout) findViewById(cn.ucai.superwechat.R.id.clear_all_history);
		...
		RelativeLayout blacklistLayout = (RelativeLayout) findViewById(cn.ucai.superwechat.R.id.rl_blacklist);
		RelativeLayout changeGroupNameLayout = (RelativeLayout) findViewById(cn.ucai.superwechat.R.id.rl_change_group_name);
		RelativeLayout idLayout = (RelativeLayout) findViewById(cn.ucai.superwechat.R.id.rl_group_id);
		idLayout.setVisibility(View.VISIBLE);
		TextView idText = (TextView) findViewById(cn.ucai.superwechat.R.id.tv_group_id_value);

		RelativeLayout rl_switch_block_groupmsg = (RelativeLayout) findViewById(cn.ucai.superwechat.R.id.rl_switch_block_groupmsg);
		switchButton = (EaseSwitchButton) findViewById(cn.ucai.superwechat.R.id.switch_btn);
		RelativeLayout searchLayout = (RelativeLayout) findViewById(cn.ucai.superwechat.R.id.rl_search);
		...
		adapter = new GridAdapter(this, cn.ucai.superwechat.R.layout.em_grid, members);
    ...
    protected void addUserToBlackList(final String username) {
        ...
        pd.setMessage(getString(cn.ucai.superwechat.R.string.Are_moving_to_blacklist));
        pd.show();
        new Thread(new Runnable() {
        	public void run() {
        		try {
        			...
        			runOnUiThread(new Runnable() {
        				public void run() {
        				    pd.dismiss();
        					Toast.makeText(getApplicationContext(), cn.ucai.superwechat.R.string.failed_to_move_into, Toast.LENGTH_SHORT).show();
        				}
        			});
        		}
        	}
	...
	private void clearGroupHistory() {
		...
		Toast.makeText(this, cn.ucai.superwechat.R.string.messages_are_empty, Toast.LENGTH_SHORT).show();
	}
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case cn.ucai.superwechat.R.id.rl_switch_block_groupmsg: // 屏蔽或取消屏蔽群组
			toggleBlockGroup();
			break;
		...
		case cn.ucai.superwechat.R.id.rl_change_group_name:
			...
			break;
		case cn.ucai.superwechat.R.id.rl_search:
	...
	private void toggleBlockGroup() {
		if(switchButton.isSwitchOpen()){
			...
			progressDialog.setMessage(getString(cn.ucai.superwechat.R.string.Is_unblock));
			progressDialog.show();
			new Thread(new Runnable() {
		        public void run() {
		            try {
		                ...
		                runOnUiThread(new Runnable() {
		                    public void run() {
		                        progressDialog.dismiss();
		                        Toast.makeText(getApplicationContext(), cn.ucai.superwechat.R.string.remove_group_of, Toast.LENGTH_LONG).show();
		                    }
		                });
	...
	private class GridAdapter extends ArrayAdapter<String> {
		...
		public View getView(final int position, View convertView, final ViewGroup parent) {
			...
			if (position == getCount() - 1) {
				...
				// 设置成删除按钮
			    holder.imageView.setImageResource(cn.ucai.superwechat.R.drawable.em_smiley_minus_btn);
//				button.setCompoundDrawablesWithIntrinsicBounds(0, R.drawable.smiley_minus_btn, 0, 0);
			    ...
			    holder.textView.setText("");
			    holder.imageView.setImageResource(cn.ucai.superwechat.R.drawable.em_smiley_add_btn);
