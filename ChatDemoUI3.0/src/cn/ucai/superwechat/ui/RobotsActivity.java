...
public class RobotsActivity extends BaseActivity {
	...
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		setContentView(cn.ucai.superwechat.R.layout.em_fragment_robots);
		inputMethodManager = (InputMethodManager) getSystemService(INPUT_METHOD_SERVICE);
		ListView mListView = (ListView) findViewById(cn.ucai.superwechat.R.id.list);
			...
			public void onSuccess(final List<EMContact> value) {
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						// save it in cache
						SuperWeChatHelper.getInstance().setRobotList(mMap);
						// save it in database
	...
	class RobotAdapter extends ArrayAdapter<RobotUser> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			if (convertView == null) {
				convertView = inflater.inflate(cn.ucai.superwechat.R.layout.em_row_robots, parent, false);
			}
